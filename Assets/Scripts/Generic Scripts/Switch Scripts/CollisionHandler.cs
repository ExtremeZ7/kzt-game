...
public class CollisionHandler : MonoBehaviour
{
    ...
    //static fields
    private static bool exclusiveSwitch;
    ...
    //methods
    /// <summary>Called by a derived class whenever its trigger is called</summary>
    /// <param name=”triggerObj”>The object that caused the trigger</param>
    ///
    protected void Trigger(GameObject triggerObj)
    {
        //Return if the object tag is not in the filter
        if (!TagFilter.Contains(triggerObj.tag))
        {
            return;
        }
    ...
    /// <summary>Usually called by a derived class whenever its trigger is exited</summary>
    /// <param name=”triggerObj”>The object that caused the trigger</param>
    ///
    protected void ExitTrigger(GameObject triggerObj)
    {
        //Return if this is not the same object that caused the latest trigger
        if (triggerObj != this.triggerObj)
        {
            return;
        }
    ...
    ///
    private IEnumerator SetSingleFrameBits(bool active = true)
    {
        if (active)
        {
    ...
    ///
    private IEnumerator DeactivateWhenFrameEnds()
    {
        yield return new WaitForEndOfFrame();
        active = false;
    }
    ...
    void OnValidate()
    {
        if (switchName == "")
        {
            switchName = gameObject.name + this.GetHashCode();
        }
    }
}