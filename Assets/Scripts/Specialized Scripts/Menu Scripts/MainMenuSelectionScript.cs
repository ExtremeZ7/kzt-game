        ...
        switch (selectionState)
        {
            ...
            case SelectionState.SelectingLoadGame:
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    //Draw Select Bar
                    GUI.DrawTexture(new Rect(20, 120 + (130 * i), 760, 120),	Helper.res.LoadTexture(loadGameSelection == i) as Texture);

                    //Draw Save Icon
                    GUI.DrawTexture(new Rect(30, 130 + (130 * i), 100, 100), Helper.res.SaveIcon(currentWorld, currentLevel) as Texture);
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        for (int k = 0; k < 3; k++)
                        {
                            GUI.DrawTexture(new Rect(140 + (38 * j), 125 + (36 * k) + (130 * i), 18, 18), 
                                Helper.res.RedGemIcon(saveFileExists && saveFiles[i].gemsCollected[j, k]) as Texture);
                            GUI.DrawTexture(new Rect(158 + (38 * j), 125 + (36 * k) + (130 * i), 18, 18), 
                                Helper.res.KIcon(saveFileExists && saveFiles[i].lettersCollected[j, k, 0]) as Texture);
                            GUI.DrawTexture(new Rect(140 + (38 * j), 143 + (36 * k) + (130 * i), 18, 18), 
                                Helper.res.KIcon(saveFileExists && saveFiles[i].lettersCollected[j, k, 1]) as Texture);
                            GUI.DrawTexture(new Rect(158 + (38 * j), 143 + (36 * k) + (130 * i), 18, 18), 
                                Helper.res.KIcon(saveFileExists && saveFiles[i].lettersCollected[j, k, 2]) as Texture);
                        }
                    }
